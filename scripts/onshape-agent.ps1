...
if ([string]::IsNullOrWhiteSpace($pythonPath) -or [string]::IsNullOrWhiteSpace($repoRoot)) {
    throw "Onshape Engineering Agent install state is incomplete: $statePath"
}
if (-not (Test-Path -LiteralPath $pythonPath -PathType Leaf)) {
    throw "Installed Python runtime is missing: $pythonPath"
}
if (-not (Test-Path -LiteralPath $repoRoot -PathType Container)) {
    throw "Installed repository is missing: $repoRoot"
}
...
# Resolve the package and its default paths from the install state, not from cwd.
